...
static const Exynos4210Reg exynos4210_clk_regs[] = {
    {"CLK_SRC_LEFTBUS",               0x4200, 0x00000000},
    {"CLK_MUX_STAT_LEFTBUS",          0x4400, 0x00000001},
    {"CLK_DIV_LEFTBUS",               0x4500, 0x00000000},
    {"CLK_DIV_STAT_LEFTBUS",          0x4600, 0x00000000},
    {"CLK_GATE_IP_LEFTBUS",           0x4800, 0xffffffff},
    {"CLKOUT_CMU_LEFTBUS",            0x4a00, 0x00010000},
    {"CLKOUT_CMU_LEFTBUS_DIV_STAT",   0x4A04, 0x00000000},
    {"CLK_SRC_RIGHTBUS",              0x8200, 0x00000000},
    ...
    */
    ...
    {"CLK_SRC_CPU",                  0x14200, 0x00000000},
    {"CLK_MUX_STAT_CPU",             0x14400, 0x00110101},
    {"CLK_DIV_CPU0",                 0x14500, 0x00000000},
    {"CLK_DIV_CPU1",                 0x14504, 0x00000000},
    {"CLK_DIV_STAT_CPU0",            0x14600, 0x00000000},
    {"CLK_DIV_STAT_CPU1",            0x14604, 0x00000000},
    {"CLK_GATE_SCLK_CPU",            0x14000, 0xffffffff},
    {"CLK_GATE_IP_CPU",              0x14900, 0xffffffff},
    /*
    {"",                 0x14000, 0x00000000},
    {"",                 0x14000, 0x00000000},
    {"",                 0x14000, 0x00000000},
                                    ...
                                    unsigned size)
{
    ...
    for (i = 0; i < EXYNOS4210_REGS_NUM; i++) {
        if (regs->offset == offset) {
            qemu_log_mask(LOG_GUEST_ERROR, "%s: %s [0x%04x] -> 0x%04x\n",
                          __func__, regs->name, (uint32_t)offset, s->reg[i]);
            return s->reg[i];
        }
        regs++;
    }
                                 ...
                                 uint64_t val, unsigned size)
{
    ...
    for (i = 0; i < EXYNOS4210_REGS_NUM; i++) {
        if (regs->offset == offset) {
            //qemu_log_mask(LOG_GUEST_ERROR, "%s: %s <0x%04x> <- 0x%04x\n",
            //        __func__, regs->name, (uint32_t)offset, (uint32_t)val);
            s->reg[i] = val;
            return;
        }
        regs++;
